...
class Metric:
    def __init__(self, config, md):
        ...
        self.eye = torch.eye(self.num_particles, device=config['system']['device']).unsqueeze(0)
        self.charge_matrix = torch.tensor(
            md.charge_matrix, dtype=torch.float, device=config['system']['device']
        ).unsqueeze(0)
        ...
        if config['molecule']['name'] == "alanine":
            self.angle_2 = torch.tensor(
                [1, 6, 8, 14], dtype=torch.long, device=config['system']['device']
            )
            self.angle_1 = torch.tensor(
                [6, 8, 14, 16], dtype=torch.long, device=config['system']['device']
            )
        elif config['molecule']['name'] == "histidine":
            self.angle_2 = torch.tensor(
                [0, 6, 8, 11], dtype=torch.long, device=config['system']['device']
            )
            self.angle_1 = torch.tensor(
                [6, 8, 11, 23], dtype=torch.long, device=config['system']['device']
            )
